...
export const ComparingPage = ({ dbUrl }) => {
  ...
  const getData = async () => {
    ...
    const [
      ...
      pricesSnapshot,
      categoriesSnapshot,
    ] = await Promise.all([getStores, getProducts, getPrices, getCategories]);
    ...
    const comparisonTable = pricesData.map((price) => {
      ...
      const product = productsData.find(
        (product) => product.id === price.id_product.replaceAll(" ", "")
      );
      const category = categoriesData.find(
        (category) => category.id === product.id_category.replaceAll(" ", "")
      );
      const store = storesData.find(
        (store) => store.id === price.id_store.replaceAll(" ", "")
      );
